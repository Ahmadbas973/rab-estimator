            ...
            <tbody>
                ...
                @if($kategori->items && $kategori->items->count() > 0)
                    ...
                    <tr data-item-id="{{ $item->id }}">
                        ...
                        <td class="py-2 px-4 border">{{ number_format($item->harga_satuan, 2, ',', '.') }}</td>
                        <td class="py-2 px-4 border">{{ number_format($item->harga_total, 2, ',', '.') }}</td>
                        <td class="py-2 px-4 border">
                            <!-- Tombol aksi untuk item, misalnya edit/hapus item -->
                            <button type="button" onclick="openVolumeModal('{{ $item->satuan }}', '{{ $item->uraian_item }}', {{ $item->id }})" class="bg-yellow-500 hover:bg-yellow-700 text-white px-2 py-1 rounded">
                                <i class="fas fa-edit"></i> Edit Volume
                            </button>
                            <button type="button" onclick="editAhsp({{ $item->id }})" class="bg-blue-500 hover:bg-blue-700 text-white px-2 py-1 rounded">                                
...
@section('scripts')
@vite(['resources/js/rab.js', 'resources/js/volume.js'])
@endsection